...
{
  const policyFilepath = fixtures.path('policy-manifest', 'onerror-exit-last-module.json');
  const mainModuleBypass = fixtures.path('policy-manifest', 'main-module-proto-bypass-last-module.js');
  const result = spawnSync(process.execPath, [
    '--experimental-policy',
    policyFilepath,
    mainModuleBypass,
