  ...
  if (experimentalPermission) {
    ...
    for (const flag of warnFlags) {
      if (getOptionValue(flag)) {
        process.emitWarning(
          `The flag ${flag} must be used under extremely cautious. ` +
    ...
    for (const flag of availablePermissionFlags) {
      if (getOptionValue(flag)) {
        throw new ERR_INCOMPATIBLE_OPTION_WITHOUT_PAIR(flag, '--experimental-permission');
      }
    }
  }
}
